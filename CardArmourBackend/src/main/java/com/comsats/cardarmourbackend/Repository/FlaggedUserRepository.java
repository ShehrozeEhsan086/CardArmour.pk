...
public interface FlaggedUserRepository extends JpaRepository<FlaggedUser,Integer> {
    ...
    @Query(value="SELECT customer.customerid,customer.name,customer.gender,customer.cnic,customer.dob,customer.age,customer.email,customer.cityid,customer.userid FROM \"Flagged User\" join customer on customer.customerid = \"Flagged User\".customer_customerid",nativeQuery = true)
    List<Customer> getAllFlaggedCustomers();
